  ...
  template: `
    <div class="min-h-screen bg-green-50 flex flex-col items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm border border-green-100">
        ...
        @if (phase() === null) {
          <div>
            <label class="block text-green-700 font-semibold mb-2 text-sm">Anaranao:</label>
            <input
              [(ngModel)]="playerName"
              type="text"
              placeholder="Anarana..."
        ...
        @if (phase() === 'join') {
          <div>
            ...
            <label class="block text-green-700 font-semibold mb-2 text-sm">Code ny vondrona:</label>
            <input
              [(ngModel)]="joinCode"
              type="text"
              placeholder="XXXXXX"
        ...
        <!-- creating spinner -->
        @if (phase() === 'creating') {
          <div class="text-center py-4 text-green-500 animate-pulse">Mamorona vondrona...</div>
        }
...
export class LobbyComponent {
  ...
  async createRoom() {
    ...
    if (error) {
      this.errorMsg.set('Tsy afaka namorona vondrona. Andramo indray.');
      this.phase.set(null);
      return;
    }
  ...
  async joinRoom() {
    ...
    if (error || !data) {
      this.errorMsg.set('Tsy hita ilay vondrona. Jereo ny teny miafina.');
      this.loading.set(false);
      return;
    }
    if (data['state'] !== 'waiting') {
      this.errorMsg.set('Efa nanomboka ny lalao ao.');
      this.loading.set(false);
      return;
    }
